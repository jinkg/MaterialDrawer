...
public class DepthFAB extends AppCompatImageButton implements Checkable {
    ...
    private void initView(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            depth = arr.getDimension(R.styleable.DepthView_depth,
                    DEFAULT_THICKNESS * getResources().getDisplayMetrics().density);
...
//        });
    ...
    public DepthFAB(Context context) {
        super(context);
        initView(null);
    }

    public DepthFAB(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(attrs);
    }

    public DepthFAB(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    class CustomShadow {
        ...
        public boolean calculateBounds(DepthFAB target) {
            ...
            float[] dst = new float[]{-padding, -padding, target.getWidth() + padding,
                    -padding, -padding, target.getHeight() + padding,
                    target.getWidth() + padding, target.getHeight() + padding};
            Matrix matrix = getMatrix();
            matrix.mapPoints(src, dst);
        ...
        public void drawShadow(Canvas canvas, DepthFAB dl, Drawable shadow) {
            ...
            float[] dst = new float[]{topLeftBack.x, topLeftBack.y,
                    topRightBack.x, topRightBack.y, bottomRightBack.x,
