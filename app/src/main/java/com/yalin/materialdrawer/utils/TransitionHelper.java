...
import com.yalin.materialdrawer.interpolators.BackOut;
import com.yalin.materialdrawer.interpolators.CircInOut;
import com.yalin.materialdrawer.interpolators.ExpoIn;
import com.yalin.materialdrawer.interpolators.ExpoOut;
import com.yalin.materialdrawer.interpolators.QuadInOut;
import com.yalin.materialdrawer.interpolators.QuintInOut;
import com.yalin.materialdrawer.interpolators.QuintOut;
import com.yalin.materialdrawer.weight.DepthFAB;
import com.yalin.materialdrawer.weight.DepthLayout;
...
public class TransitionHelper {
    ...
    public static void startIntroAnim(View root, AnimatorListenerAdapter introEndListener) {
        ...
        introAnimate((DepthFAB) root.findViewById(R.id.fab_container), MOVE_Y_STEP * 2f, 20f, 45, 190);
    ...
    static ObjectAnimator introAnimate(final DepthFAB target, final float moveY, final float customElevation, long delay, int subtractDelay) {
        target.setPivotY(getDistanceToCenter(target));
        target.setPivotX(getDistanceToCenterX(target));
        target.setCameraDistance(10000 * target.getResources().getDisplayMetrics().density);
    ...
    static ValueAnimator exitAnimate(final DepthLayout target, final float moveY,
                                     final float customElevation, long delay,
                                     int subtractDelay, boolean continueOffscreen) {
        target.setPivotY(getDistanceToCenter(target));
        target.setPivotX(getDistanceToCenterX(target));
        target.setCameraDistance(10000 * target.getResources().getDisplayMetrics().density);

        ObjectAnimator rotationX = ObjectAnimator.ofFloat(target,
                View.ROTATION_X, TARGET_ROTATION_X).setDuration(DURATION);
        ...
        ObjectAnimator elevation = ObjectAnimator.ofFloat(target, "CustomShadowElevation",
                ...
                customElevation * target.getResources().getDisplayMetrics().density).setDuration(DURATION);
    ...
    static ValueAnimator exitAnimate(final DepthFAB target, final float moveY,
                                     final float customElevation, long delay, int subtractDelay,
                                     boolean continueOffscreen) {
        target.setPivotY(getDistanceToCenter(target));
        target.setPivotX(getDistanceToCenterX(target));
        target.setCameraDistance(10000 * target.getResources().getDisplayMetrics().density);

        ObjectAnimator rotationX = ObjectAnimator.ofFloat(target, View.ROTATION_X,
                TARGET_ROTATION_X).setDuration(DURATION);
        rotationX.setInterpolator(VALUEinterpolator);
        rotationX.setStartDelay(delay);
        rotationX.start();
        ...
        ObjectAnimator rotation = ObjectAnimator.ofFloat(target, View.ROTATION,
                TARGET_ROTATION).setDuration(1600);
        rotation.setInterpolator(VALUEinterpolator);
        rotation.setStartDelay(delay);
        rotation.start();
    ...
    public static void animateToMenuState(View root, AnimatorListenerAdapter onMenuAnimFinished) {
        exitAnimate((DepthLayout) root.findViewById(R.id.ma_appBar),
                MOVE_Y_STEP, 20f, 30, 170, false);
        exitAnimate((DepthLayout) root.findViewById(R.id.layout_container),
                0, 30f, 15, 190, false);
        exitAnimate((DepthFAB) root.findViewById(R.id.fab_container),
                MOVE_Y_STEP * 2f, 20f, 45, 210, false);
        exitAnimate((DepthLayout) root.findViewById(R.id.container_text),
                MOVE_Y_STEP, 20f, 60, 230, false);
...
//                        | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN);
    ...
    public static void animateMenuOut(View root) {
        ...
        continueOutToRight((DepthFAB) root.findViewById(R.id.fab_container), MOVE_Y_STEP * 2f, 40);
    ...
    public static void startRevertFromMenu(View root,
        ...
        revertFromMenu((DepthFAB) root.findViewById(R.id.fab_container), 20f, 20, FAB_ELEVATION);
        ...
        ObjectAnimator translationY = ObjectAnimator.ofFloat(root,
                View.TRANSLATION_Y, 0).setDuration(DURATION);
    ...
    static ObjectAnimator revertFromMenu(final DepthLayout target, final float customElevation,
        ...
        ObjectAnimator translationY = ObjectAnimator.ofFloat(target,
                View.TRANSLATION_Y, 0).setDuration(700);
        translationY.setInterpolator(new BackOut());
        translationY.setStartDelay(250 + FISRTDELAY + subtractDelay);
        translationY.start();

        ObjectAnimator rotationX = ObjectAnimator.ofFloat(target, View.ROTATION_X,
                target.getRotationX(), 0).setDuration(1000);
        rotationX.setInterpolator(new QuintInOut());
        ...
        elevation.start();
        target.setCustomShadowElevation(customElevation
                * target.getResources().getDisplayMetrics().density);

        ObjectAnimator scaleX = ObjectAnimator.ofFloat(target, View.SCALE_X,
                target.getScaleX(), 1f).setDuration(1000);
        scaleX.setInterpolator(new CircInOut());
        ...
        ObjectAnimator scaleY = ObjectAnimator.ofFloat(target, View.SCALE_Y,
                target.getScaleY(), 1f).setDuration(1000);
        scaleY.setInterpolator(new CircInOut());
        ...
        ObjectAnimator rotation = ObjectAnimator.ofFloat(target, View.ROTATION,
                target.getRotation(), 0).setDuration(1100);
    ...
    static ObjectAnimator revertFromMenu(final DepthFAB target, final float customElevation,
                                         int subtractDelay, float targetElevation) {
        target.setPivotY(getDistanceToCenter(target));
        target.setPivotX(getDistanceToCenterX(target));
        target.setCameraDistance(10000 * target.getResources().getDisplayMetrics().density);
    ...
    @NonNull
    private static AnimatorListenerAdapter getShowStatusBarListener(final DepthFAB target) {
        return new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                super.onAnimationEnd(animation);
